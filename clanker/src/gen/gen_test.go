...
func TestRun_WritesEachTargetFile(t *testing.T) {
	...
	if err := gen.Run(root, cmds, nil, nil, nil, target.Registry()); err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRun_MergesAgentIntoOpencodeJSON(t *testing.T) {
	...
	if err := gen.Run(root, nil, agents, nil, nil, target.Registry()); err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRun_ErrorsWhenNotDotfilesRoot(t *testing.T) {
	...
	err := gen.Run(root, []spec.Command{{Name: "x"}}, nil, nil, nil, target.Registry())
	if err == nil {
