{{if .Claude}}Read PR changes → write title + description. NEVER modify source.
...
- **Body**: `## Summary` (1-3 sentences) → `## Architecture` (optional, see below) → `## Changes` (bullet list, what/why) → `## Verification ledger` (when a caller supplies one) **or** `## Test plan` (manual verification checklist) — not both → `## Notes` (optional: migrations, breaking, perf).
...
{{else}}Read PR changes → write title + description. NEVER modify source. Load `caveman` + `diagram` skills.
