...
Creating a PR (`gh pr create`, `/ship`, ad-hoc) → delegate title/body to {{if .Claude}}`pr-describer`; NEVER inline{{else}}the `pr-describer` agent; NEVER write it inline{{end}}. It owns the flow + standard structure + `diagram` skill. After a `git push` with an open PR, {{if .Claude}}refresh the now-stale description the same way — `pr-refresh-reminder` hook flags it.{{else}}its description is stale — refresh it the same way (delegate to `pr-describer`).{{end}}

## TDD

Test-driven by default for ALL feature/bugfix/refactor/behavioural-narrowing work, every project & agent — incl. subtractive changes (failing test asserting the new contract before stripping source; never skip because it only removes code). Bugs: failing test first. Never refactor without tests covering current behavior. Write the minimum code the test requires. Error/validation tests assert BOTH status AND message/field (`assertStatus(422)` + `errors.<field>`). Load the `tdd` skill for the full protocol (red-green-refactor, rollback, contract migration, test quality).{{if .Claude}} `tdd-reminder` hook nudges on source edits with no test in flight.{{end}}
...
- **Diagnosis:** bare problem + artifact (log/CI link/error) with no hypothesis → lead with your hypotheses + the cheapest discriminating check, expose the reasoning, invite me to predict before you fix — never a black-box answer.{{if .Claude}} (`offloading-nudge` hook.){{end}}
- **Delegated build:** non-trivial work I didn't watch → don't declare done on green. Incremental comprehension checkpoint (what it does, how it fits, the key decision, the riskiest seam) as active recall — make me predict, don't lecture; route complex parts to `explain`/`tutor`; flag a growing ball-of-mud and offer `/simplify`.{{if .Claude}} (`comprehension-nudge` hook.){{end}}
...
A "yes" authorizes one understood thing. Multi-part choices → separate {{if .Claude}}`AskUserQuestion` items{{else}}per-item questions{{end}}, never a free-text "do X and Y?". Before acting, restate what the approval covers; anything unclear is confirmed separately. Irreversible/outward-facing actions (commit, push, delete, send, publish) each get their own confirmation.{{if .Claude}} `approval-scope` hook flags bare approvals.{{end}}
