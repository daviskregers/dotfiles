...
- A diagram is never the whole answer. Every C4 altitude draws boxes that are services or processes, so a perfect one still never says what the code does. Follow it with the code-level walk: named file + function per hop along the entrypoint → hops → line route, and what the change does inside each hop — the branch added, the call rerouted, the field now written. Diagram = where it happens; walk = what happens. Missing the walk, I can't review the diff from the artifact.
