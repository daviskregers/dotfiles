Entry ritual for starting a piece of work. Context: {{.Args}} — none? Ask what they're starting on. Ask §0, then run the floor (§1) whatever the answer. Do NOT run ahead to code.
...
3. **Slices of ≤__CHURN_LIMIT__ lines, each one red→green.** Sketch them with churn guesses before any code; over the limit = candidate to split. Test-first per slice, no exceptions — see `tdd`.
