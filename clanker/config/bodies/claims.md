Test the Predictions the user wrote in an issue's notes file, so they find out how wrong they were. Notes file: {{.Args}} — none? Resolve from cwd: `.dk-notes/<ID>-<slug>/notes.md`, most recently modified. Several candidates → ask which; none → stop, there is nothing to score.
