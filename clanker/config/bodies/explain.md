{{if .Claude}}Use explainer agent. Generate visual HTML explanation built on the shared `report` scaffold (dark theme + light toggle, glossary/citation hovercards, in-memory annotation tool, zoomable Mermaid with legend) plus quizzes.

Topic: {{.Args}}

No topic? Ask user what to explain.

When the explainer agent returns the saved path, **automatically open it in the browser** (`open <path>` on macOS, `xdg-open` on Linux) — don't make the user open it.
{{else}}Generate visual explanation. Create self-contained HTML page helping user understand concept. NEVER modify source code files.
