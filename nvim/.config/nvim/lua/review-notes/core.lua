...
M.inputs = {
    markdown = require("review-notes.inputs.markdown"),
    pr = require("review-notes.inputs.pr"),
}

M.outputs = {
    markdown = require("review-notes.outputs.markdown"),
    pr = require("review-notes.outputs.pr"),
    quickfix = require("review-notes.outputs.quickfix"),
    agent = require("review-notes.outputs.agent"),
}
...
local function format_note_for_agent(n)
    local range = (n.start_line == n.end_line) and tostring(n.start_line)
        or string.format("%d-%d", n.start_line, n.end_line)
    local header = string.format("%s:%s", n.file, range)
    if n.author then
        header = header .. " (" .. n.author .. ")"
    end
    return header .. "\n" .. (n.text or "")
end

local function send_with_prefix(prefix, note)
    require("agents.core").send(prefix .. format_note_for_agent(note))
end
...
local function refresh_after_change()
    ...
    if ctx then
        M.refresh_extmarks(buf, ctx.file, ctx.side)
    end
end

local function preview_lines_for(note)
    if note.context_line and #note.context_line > 0 then
        return note.context_line
    end
    return { string.format("%s:%d (%s)", note.file, note.start_line, note.kind or "note") }
end

M.actions = {
    ["/comment"] = function(note)
        send_with_prefix("/comment ", note)
    end,
    ["/explain"] = function(note)
        send_with_prefix("/explain ", note)
    end,
    ["send raw"] = function(note)
        send_with_prefix("", note)
    end,
    ["goto source"] = function(note)
        M.goto_note(note)
    end,
    ["edit"] = function(note)
    ...
    ["reply"] = function(note)
        ...
        if note.in_reply_to_id then
            for _, n in ipairs(state.notes) do
                if n.pr_comment_id == note.in_reply_to_id then
                    root = n
                    break
                end
            end
        ...
        M.open_comment_float({
            preview_lines = {
                string.format("Reply to %s:%d (@%s)", root.file, root.start_line, root.author or "?"),
            },
            initial_text = "",
            on_confirm = function(text)
    ...
    ["resolve thread"] = function(note)
        ...
        local ok = require("review-notes.inputs.pr").resolve_thread(target.thread_id)
        if ok then
            -- Mark all notes in this thread as resolved locally so UI matches.
            for _, n in ipairs(state.notes) do
                if n.thread_id == target.thread_id then
                    n.resolved = true
                end
            end
            refresh_after_change()
...
function M.get_thread(note)
    ...
    if not root.pr_comment_id then
        return { root }
    end

    local replies = {}
    for _, n in ipairs(state.notes) do
        if n ~= root and (n.in_reply_to_id == root.pr_comment_id or n.replies_to_id == root.pr_comment_id) then
...
function M.format_markdown(notes, git_meta)
    ...
    add("# Review Notes")
    add("")
    if git_meta.branch then
        add("- **Branch:** `" .. git_meta.branch .. "`")
    end
    if git_meta.ref then
        add("- **Ref:** `" .. git_meta.ref .. "`")
    end
    ...
    local function render_note(note, file)
        ...
        add("```" .. ext)
        for _, l in ipairs(note.context_before or {}) do
            add(l)
        end
        if note.start_line == note.end_line then
        ...
        for _, l in ipairs(note.context_line or {}) do
            add(l)
        end
        for _, l in ipairs(note.context_after or {}) do
            add(l)
        end
        add("```")
        add("")
...
function M.place_extmark(bufnr, note)
    ...
    if note.start_line > line_count then
        return
    end
    ...
    if #first_line > max then
        truncated = truncated .. "..."
    end
    if #note.text > #first_line then
        truncated = truncated .. " …"
    end
    if note.resolved then
        truncated = "✓ " .. truncated
...
function M.peek_note_at_cursor()
    ...
    if not ctx then
        return vim.notify("Not a reviewable buffer", vim.log.levels.WARN)
    end
    local line = vim.api.nvim_win_get_cursor(0)[1]
    local note = M.get_note_at_line(ctx.file, line, ctx.side)
    if not note then
        return vim.notify("No note at cursor", vim.log.levels.INFO)
    end
    ...
    local function add_note_block(n, is_reply)
        ...
        if is_reply and n.kind == "reply" then
            kind_tag = " (pending reply)"
        end
        table.insert(lines, string.format("%s:%d%s%s", n.file, n.start_line, sev, kind_tag))
        if n.author then
            table.insert(lines, "by @" .. n.author)
        end
    ...
    for _, l in ipairs(lines) do
        height = height + rows_after_wrap(l)
    end
...
function M.notes_with_live_positions()
    ...
    for _, note in ipairs(state.notes) do
        ...
        if note._extmark_buf and note._extmark_id and vim.api.nvim_buf_is_valid(note._extmark_buf) then
            local pos = vim.api.nvim_buf_get_extmark_by_id(note._extmark_buf, state.ns_id, note._extmark_id, {})
