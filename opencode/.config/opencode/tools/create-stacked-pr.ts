...
export default tool({
    ...
    args: {
        base: tool.schema.string().optional().describe("Explicit base branch (skips parent detection)"),
        title: tool.schema.string().optional().describe("PR title (default: last commit subject)"),
        body: tool.schema
            .string()
            .optional()
            .describe("PR body markdown (default: the commit list); AI attribution is appended either way"),
        draft: tool.schema.boolean().optional().describe("Open the PR as a draft"),
    },
    execute,
})
