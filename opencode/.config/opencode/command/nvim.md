...
Targets: $ARGUMENTS — if paths are given, use them verbatim. Otherwise infer the distinct directories the subagents / workflow agents in THIS session were dispatched to work on (e.g. you sent one to `../frontend`). Can't tell which? Ask; don't guess.
