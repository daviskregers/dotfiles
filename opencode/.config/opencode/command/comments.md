...
PR URL/number: $1 — none? Resolve current branch's PR via `gh pr view --json url,number`. No PR? Ask.
...
4. Close (default for BOTH outcomes): call `resolve-pr-thread` tool with `threadId` + `replyBody` (fixed → what changed, `path:line`; ignored → why). Posts reply then resolves.
